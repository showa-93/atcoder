...
func solve(in io.Reader, out io.Writer) {
	...
	for i := 0; i < n; i++ {
		...
		for j := 0; j < a; j++ {
			s := reader.Int() - 1
			// グラフを作成するときに、集合を直接つなぐのではなく、各集合をまとめる頂点を追加する
			// BFSで頂点間の移動が表すことができるようになる
			g[m+i] = append(g[m+i], s)
			g[s] = append(g[s], m+i)
		}
	}

	costs := new1dInt(n+m, 10000000)
	costs[0] = 0
	q := []int{0}
	for len(q) > 0 {
		a := q[0]
	...
	if costs[m-1] == 10000000 {
		writer.Int(-1)
	} else {
		writer.Int(costs[m-1]/2 - 1)
	}
}
