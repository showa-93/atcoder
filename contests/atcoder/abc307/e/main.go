...
func solve(in io.Reader, out io.Writer) {
	...
	dp := new2dInt(n+1, 2, 0)
	// dp[i番目の人][1番目の人と同じかどうか(0→false, 1→true)]
	dp[1][1] = m
	for i := 1; i < n; i++ {
		// i番目の人が1番目と異なる → i+1番目の人は、1番目の数字とi番目の数字を除いたm-2だけとりうる
		dp[i+1][0] += dp[i][0] * (m - 2)
		// i番目の人が1番目と同じ → i+1番目の人は、1番目の数字を除いたm-1だけとりうる
		dp[i+1][0] += dp[i][1] * (m - 1)
		// i+1番目の人が1番目と同じ数字になるのはi番目の人が1番目の人と同じにならなかった場合だけ
		dp[i+1][1] += dp[i][0]

		dp[i+1][0] %= mod998244353
		dp[i+1][1] %= mod998244353
	}

	// n番目の人が1番目の人と同じにならなかった場合がすべてのとりうるパターン
	// → 循環を表してる n番目→1番目
	writer.Int(dp[n][0])
}
