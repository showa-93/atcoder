...
func solve(in io.Reader, out io.Writer) {
	...
	w := NewWriter(out)
	defer w.Flush()
	a, b := float64(r.ReadInt()), float64(r.ReadInt())

	ll, rr := 0, MaxInt
	for rr-ll >= 0 {
		c := (ll + rr) / 2
		if a/math.Sqrt(float64(1+c))-a/math.Sqrt(float64(c+2)) < b {
			rr = c - 1
		} else {
			ll = c + 1
		}
	}
	w.String(fmt.Sprintf("%f", a/math.Sqrt(float64(1+ll))+float64(ll)*b))
}
