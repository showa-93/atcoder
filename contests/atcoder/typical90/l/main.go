...
func solve(in io.Reader, out io.Writer) {
	...
	defer writer.Flush()
	h, w := reader.Int(), reader.Int()
	q := reader.Int()
	ds := NewDijointSetsForests(h + w*10000)
	redMap := make(map[[2]int]struct{})
	for i := 0; i < q; i++ {
		t := reader.String()
		switch t {
		case "1":
			a := [2]int{reader.Int() - 1, reader.Int() - 1}
			redMap[a] = struct{}{}
			for _, b := range [][2]int{{1, 0}, {0, 1}, {-1, 0}, {0, -1}} {
				c := [2]int{a[0] + b[0], a[1] + b[1]}
				if c[0] < 0 || c[1] < 0 || c[0] >= h || c[1] >= w {
					continue
				}
				if _, ok := redMap[c]; ok {
					ds.Unite(a[0]+a[1]*10000, c[0]+c[1]*10000)
				}
			}
		case "2":
			a := [2]int{reader.Int() - 1, reader.Int() - 1}
			b := [2]int{reader.Int() - 1, reader.Int() - 1}
			if _, ok := redMap[a]; !ok {
				writer.String("No").Cr()
			} else if _, ok := redMap[b]; !ok {
				writer.String("No").Cr()
			} else if ds.Same(a[0]+a[1]*10000, b[0]+b[1]*10000) {
