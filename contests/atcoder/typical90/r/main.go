...
func solve(in io.Reader, out io.Writer) {
	...
	defer writer.Flush()
	t := reader.Float64()
	l, x, y := reader.Float64(), reader.Float64(), reader.Float64()
	q := reader.Int()
	for i := 0; i < q; i++ {
		e := reader.Float64()
		c := e / t
		// 観覧車の横方向の位置。逆回りなのでマイナス。
		yy := l / 2 * math.Cos(-2*math.Pi*c-math.Pi/2)
		// 観覧車のz軸。逆回りなのでマイナス。
		zz := l/2*math.Sin(-2*math.Pi*c-math.Pi/2) + l/2
		ll := math.Sqrt(math.Pow(x, 2) + math.Pow(y-yy, 2))
		p := math.Acos(ll / math.Sqrt(math.Pow(ll, 2)+math.Pow(zz, 2)))
		arg := math.Abs(180 * p / math.Pi)
		writer.Float64(arg).Cr()
	}
}
