...
func solve(in io.Reader, out io.Writer) {
	...
	for i := 1; i <= n; i++ {
		c, p := reader.Int()-1, reader.Int()
		classResult[c][i] = p
		classResult[0][i] += classResult[0][i-1]
		classResult[1][i] += classResult[1][i-1]
	}

	q := reader.Int()
	for i := 0; i < q; i++ {
		l, r := reader.Int()-1, reader.Int()
		writer.
			Int(classResult[0][r] - classResult[0][l]).Space().
			Int(classResult[1][r] - classResult[1][l]).Cr()
	}
}
