...
func solve(in io.Reader, out io.Writer) {
	...
	same := func(a, b [][]string, pos [2]int) bool {
		for i := 0; i < h; i++ {
			for j := 0; j < w; j++ {
				if a[i+pos[0]][j+pos[1]] != b[i][j] {
					return false
				}
			}
		}
	...
	c := make([][]string, len(a)*2)
	for i := 0; i < h; i++ {
		c[i] = make([]string, len(a[i])*2)
		c[i+h] = make([]string, len(a[i])*2)
		copy(c[i][:w], a[i])
		copy(c[i][w:], a[i])
		copy(c[i+h][:w], a[i])
		copy(c[i+h][w:], a[i])
