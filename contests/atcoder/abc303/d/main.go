...
func solve(in io.Reader, out io.Writer) {
	...
	defer writer.Flush()
	x, y, z := reader.Int(), reader.Int(), reader.Int()
	s := reader.String()
	dp := new2dInt(len(s)+1, 2, MaxInt)
	dp[0][0] = 0
	dp[0][1] = -1
	calc := func(c rune, f int) int {
		xx, yy := x, y
		if f == 1 {
			xx, yy = yy, xx
		}
	...
	for i, c := range s {
		j := i + 1
		if dp[i][0] >= 0 && dp[i][0] < MaxInt {
			dp[j][0] = Min(dp[j][0], dp[i][0]+calc(c, 0))
			dp[j][1] = Min(dp[j][1], dp[i][0]+calc(c, 1)+z)
		}
		if dp[i][1] >= 0 && dp[i][1] < MaxInt {
			dp[j][0] = Min(dp[j][0], dp[i][1]+calc(c, 0)+z)
			dp[j][1] = Min(dp[j][1], dp[i][1]+calc(c, 1))
		}
	}
