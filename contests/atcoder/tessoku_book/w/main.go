...
func solve(in io.Reader, out io.Writer) {
	...
	a := new2dInt(m, n, 0)
	for i := 0; i < m; i++ {
		a[i] = reader.ReadIntLine(n)
	}

	dp := new2dInt(m+1, 1<<n, MaxInt-1)
	dp[0][0] = 0
	for i := 1; i <= m; i++ {
		for s := 0; s < 1<<n; s++ {
			// チケットを選んだ場合の集合を計算する
			...
			dp[i][s] = Min(dp[i][s], dp[i-1][s])   // チケットを選ばないパターン
			dp[i][t] = Min(dp[i][t], dp[i-1][s]+1) // チケットを選ぶパターン
		}
	}
	if v := dp[m][1<<n-1]; v != MaxInt-1 {
		writer.Int(v)
	} else {
		writer.Int(-1)
	}
