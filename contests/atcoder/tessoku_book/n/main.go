...
func solve(in io.Reader, out io.Writer) {
	...
	defer writer.Flush()
	n, k := reader.ReadInt(), reader.ReadInt()
	a := reader.ReadIntLine(n)
	b := reader.ReadIntLine(n)
	c := reader.ReadIntLine(n)
	d := reader.ReadIntLine(n)

	ab := make(map[int]struct{})
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			ab[a[i]+b[j]] = struct{}{}
		}
